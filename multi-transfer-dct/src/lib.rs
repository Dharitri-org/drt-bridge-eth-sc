    ...
    tx_batch_module::TxBatchModule + max_bridged_amount_module::MaxBridgedAmountModule
{
    ...
    #[only_owner]
    #[endpoint(batchTransferDctToken)]
    fn batch_transfer_dct_token(
        &self,
        batch_id: u64,
        ...
        for eth_tx in transfers {
            ...
            if eth_tx.to.is_zero() || self.blockchain().is_smart_contract(&eth_tx.to) {
                ...
                must_refund = true;
            } else if !self.is_local_role_set(&eth_tx.token_id, &DctLocalRole::Mint) {
            ...
            self.send()
                .dct_local_mint(&eth_tx.token_id, 0, &eth_tx.amount);
            ...
            valid_payments_list.push(DctTokenPayment::new(eth_tx.token_id, 0, eth_tx.amount));
        }
    ...
    fn is_local_role_set(&self, token_id: &TokenIdentifier, role: &DctLocalRole) -> bool {
    ...
    fn is_account_same_shard_frozen(
        ...
        let token_data = self
            .blockchain()
            .get_dct_token_data(dest_address, token_id, 0);
        token_data.frozen
    }
    ...
    fn distribute_payments(
        ...
        for (dest, p) in dest_addresses.iter().zip(payments.iter()) {
            self.send()
                .direct_dct(&dest, &p.token_identifier, 0, &p.amount);
        }
    }
