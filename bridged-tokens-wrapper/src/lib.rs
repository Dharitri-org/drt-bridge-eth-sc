...
pub trait BridgedTokensWrapper: dharitri_sc_modules::pause::PauseModule {
    #[init]
    fn init(&self) {
    ...
    fn wrap_tokens(&self) -> PaymentsVec<Self::Api> {
        ...
        let original_payments = self.call_value().all_dct_transfers().deref().clone();
        ...
        for payment in &original_payments {
            ...
            self.send()
                .dct_local_mint(&universal_token_id, 0, &converted_amount);
            new_payments.push(DctTokenPayment::new(
                universal_token_id,
                0,
                converted_amount,
            ));
    ...
    fn unwrap_token(&self, requested_token: TokenIdentifier) {
        ...
        self.send()
            .dct_local_burn(&universal_bridged_token_ids, 0, &payment_amount);

        let caller = self.blockchain().get_caller();
        self.send()
            .direct_dct(&caller, chain_specific_token_id, 0, &converted_amount);
    }
