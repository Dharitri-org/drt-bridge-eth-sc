    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    /// Maps an DCT token to an ERC20 address. Used by relayers.
    #[only_owner]
    #[endpoint(addMapping)]
    ...
    #[only_owner]
    #[endpoint(pauseDctSafe)]
    fn pause_dct_safe(&self) {
    ...
    #[only_owner]
    #[endpoint(unpauseDctSafe)]
    fn unpause_dct_safe(&self) {
        let _: IgnoreValue = self
            .get_dct_safe_proxy_instance()
            .unpause_endpoint()
            .execute_on_dest_context();
    ...
    /// This is used in the DctSafe contract to determine the fee amount
    ///
    /// fee_amount = eth_gas_limit * price_per_gas_unit
    ///
    /// where price_per_gas_unit is queried from the aggregator (fee estimator SC)
    #[only_owner]
    #[endpoint(changeDharitriToEthGasLimit)]
    fn change_dharitri_to_eth_gas_limit(&self, new_gas_limit: BigUint) {
        let _: IgnoreValue = self
            .get_dct_safe_proxy_instance()
    ...
    #[only_owner]
    #[endpoint(dctSafeAddTokenToWhitelist)]
    fn dct_safe_add_token_to_whitelist(
        &self,
    ...
    #[only_owner]
    #[endpoint(dctSafeRemoveTokenFromWhitelist)]
    fn dct_safe_remove_token_from_whitelist(&self, token_id: TokenIdentifier) {
        let _: IgnoreValue = self
            .get_dct_safe_proxy_instance()
    ...
    /// Sets maximum batch size for the DctSafe SC.
    ...
    #[only_owner]
    #[endpoint(dctSafeSetMaxTxBatchSize)]
    fn dct_safe_set_max_tx_batch_size(&self, new_max_tx_batch_size: usize) {
        let _: IgnoreValue = self
            .get_dct_safe_proxy_instance()
    ...
    /// Sets the maximum block duration in which an DctSafe batch accepts transactions
    ...
    #[only_owner]
    #[endpoint(dctSafeSetMaxTxBatchBlockDuration)]
    fn dct_safe_set_max_tx_batch_block_duration(&self, new_max_tx_batch_block_duration: u64) {
        let _: IgnoreValue = self
            .get_dct_safe_proxy_instance()
    ...
    #[only_owner]
    #[endpoint(dctSafeSetMaxBridgedAmountForToken)]
    fn dct_safe_set_max_bridged_amount_for_token(
        &self,
        token_id: TokenIdentifier,
        max_amount: BigUint,
    ) {
    ...
    #[only_owner]
    #[endpoint(multiTransferDctSetMaxBridgedAmountForToken)]
    fn multi_transfer_dct_set_max_bridged_amount_for_token(
        &self,
        token_id: TokenIdentifier,
        max_amount: BigUint,
    ) {
    ...
    #[only_owner]
    #[endpoint(multiTransferDctSetMaxRefundTxBatchSize)]
    fn multi_transfer_dct_set_max_refund_tx_batch_size(&self, new_max_tx_batch_size: usize) {
        let _: IgnoreValue = self
            .get_multi_transfer_dct_proxy_instance()
            .set_max_tx_batch_size(new_max_tx_batch_size)
            .execute_on_dest_context();
    ...
    #[only_owner]
    #[endpoint(multiTransferDctSetMaxRefundTxBatchBlockDuration)]
    fn multi_transfer_dct_set_max_refund_tx_batch_block_duration(
        &self,
    ...
    #[only_owner]
    #[endpoint(multiTransferDctSetWrappingContractAddress)]
    fn multi_transfer_dct_set_wrapping_contract_address(
        &self,
