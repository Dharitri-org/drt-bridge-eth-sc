...
pub trait UtilModule: crate::storage::StorageModule {
    ...
    #[proxy]
    fn dct_safe_proxy(&self, sc_address: ManagedAddress) -> dct_safe::Proxy<Self::Api>;

    #[proxy]
    fn multi_transfer_dct_proxy(
        &self,
        sc_address: ManagedAddress,
    ) -> multi_transfer_dct::Proxy<Self::Api>;

    fn get_dct_safe_proxy_instance(&self) -> dct_safe::Proxy<Self::Api> {
        self.dct_safe_proxy(self.dct_safe_address().get())
    }

    fn get_multi_transfer_dct_proxy_instance(&self) -> multi_transfer_dct::Proxy<Self::Api> {
        self.multi_transfer_dct_proxy(self.multi_transfer_dct_address().get())
    }
}
