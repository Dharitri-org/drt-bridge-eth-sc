...
setLocalRolesMultiTransferDct() {
    CHECK_VARIABLES DCT_SYSTEM_SC_ADDRESS CHAIN_SPECIFIC_TOKEN MULTI_TRANSFER

    mxpy --verbose contract call ${DCT_SYSTEM_SC_ADDRESS} --recall-nonce --pem=${ALICE} \
    --gas-limit=60000000 --function="setSpecialRole" \
    --arguments str:${CHAIN_SPECIFIC_TOKEN} ${MULTI_TRANSFER} str:DCTRoleLocalMint \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}

unsetLocalRolesMultiTransferDct() {
    CHECK_VARIABLES DCT_SYSTEM_SC_ADDRESS CHAIN_SPECIFIC_TOKEN MULTI_TRANSFER
