...
addTokenToWhitelist() {
    ...
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=60000000 --function="dctSafeAddTokenToWhitelist" \
...
removeTokenFromWhitelist() {
    ...
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=60000000 --function="dctSafeRemoveTokenFromWhitelist" \
...
dctSafeSetMaxTxBatchSize() {
    ...
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=30000000 --function="dctSafeSetMaxTxBatchSize" \
...
dctSafeSetMaxTxBatchBlockDuration() {
    ...
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=30000000 --function="dctSafeSetMaxTxBatchBlockDuration" \
...
pauseDctSafe() {
    ...
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=40000000 --function="pauseDctSafe" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
unpauseDctSafe() {
    ...
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=40000000 --function="unpauseDctSafe" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
dctSafeSetMaxBridgedAmountForToken() {
    ...
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=40000000 --function="dctSafeSetMaxBridgedAmountForToken" \
...
multiTransferDctSetMaxBridgedAmountForToken() {
    ...
    MAX=$(echo "scale=0; $MAX_AMOUNT*10^$NR_DECIMALS_CHAIN_SPECIFIC/1" | bc)
    mxpy --verbose contract call ${MULTISIG} --recall-nonce --pem=${ALICE} \
    --gas-limit=40000000 --function="multiTransferDctSetMaxBridgedAmountForToken" \
    --arguments str:${CHAIN_SPECIFIC_TOKEN} ${MAX} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
