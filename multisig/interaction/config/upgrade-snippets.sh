...
deploySafeForUpgrade() {
    ...
    local DCT_SAFE_ETH_TX_GAS_LIMIT=20000 # gives us 200$ for dharitri->eth

    mxpy --verbose contract deploy --project=${PROJECT_SAFE} --recall-nonce --pem=${ALICE} \
    --gas-limit=150000000 \
    --arguments 0x${AGGREGATOR_ADDRESS_HEX} ${DCT_SAFE_ETH_TX_GAS_LIMIT} \
...
upgradeSafeContract() {
    getDctSafeAddressHex
    getAggregatorAddressHex
    local DCT_SAFE_ETH_TX_GAS_LIMIT=20000
    ...
    --arguments 0x${DCT_SAFE_ADDRESS_HEX} 0x${NEW_SAFE_ADDR} 0x00 \
    0x${AGGREGATOR_ADDRESS_HEX} ${DCT_SAFE_ETH_TX_GAS_LIMIT} \
...
upgradeMultisig() {
    ...
    --arguments 0x${DCT_SAFE_ADDRESS_HEX} 0x${MULTI_TRANSFER_DCT_ADDRESS_HEX} \
