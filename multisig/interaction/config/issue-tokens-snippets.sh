DCT_ISSUE_COST=50000000000000000

issueUniversalToken() {
    CHECK_VARIABLES DCT_SYSTEM_SC_ADDRESS DCT_ISSUE_COST UNIVERSAL_TOKEN_DISPLAY_NAME \
    UNIVERSAL_TOKEN_TICKER NR_DECIMALS_UNIVERSAL

    mxpy --verbose contract call ${DCT_SYSTEM_SC_ADDRESS} --recall-nonce --pem=${ALICE} \
    --gas-limit=60000000 --value=${DCT_ISSUE_COST} --function="issue" \
...
issueChainSpecificToken() {
    CHECK_VARIABLES DCT_SYSTEM_SC_ADDRESS DCT_ISSUE_COST CHAIN_SPECIFIC_TOKEN_DISPLAY_NAME \
...
transferToSC() {
    ...
    --gas-limit=5000000 --function="DCTTransfer" \
...
setMintRole() {
    ...
    --arguments str:${CHAIN_SPECIFIC_TOKEN} ${ALICE_ADDRESS} str:DCTRoleLocalMint \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
mint() {
    ...
    --gas-limit=300000 --function="DCTLocalMint" \
