...
ADDRESS=$(mxpy data load --key=address-testnet-dct-safe)
...
deploy() {
    ...
    --gas-limit=100000000 \
    --arguments ${ETHEREUM_FEE_PREPAY_SC_ADDRESS} ${WRAPPED_MOAX_TOKEN_ID} ${WRAPPED_ETH_TOKEN_ID} \
    ...
    mxpy data store --key=address-testnet-dct-safe --value=${ADDRESS}
...
setLocalRolesWrappedMoax() {
    ...
    --arguments ${WRAPPED_MOAX_TOKEN_ID} ${ADDRESS_HEX} ${LOCAL_BURN_ROLE} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
createTransaction() {
    ...
    local TOKEN_USED_FOR_TX_FEES = 0x4d4f4158 # "MOAX"
    
    mxpy --verbose contract call ${ADDRESS} --recall-nonce --pem=${BOB} \
    --gas-limit=50000000 --function="DCTTransfer" \
    --arguments ${WRAPPED_MOAX_TOKEN_IDENTIFIER} 0x64 ${CREATE_TRANSACTION_ENDPOINT} ${DEST_ADDRESS} ${TOKEN_USED_FOR_TX_FEES} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
