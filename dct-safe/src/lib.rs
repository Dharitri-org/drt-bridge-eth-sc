    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    fn set_transaction_batch_status(
        ...
        for (tx, tx_status) in tx_batch.iter().zip(tx_statuses.to_vec().iter()) {
            // Since tokens don't exist in the DctSafe in the case of a refund transaction
            ...
            match tx_status {
                TransactionStatus::Executed => {
                    ...
                    if self.is_local_role_set(&tx.token_identifier, &DctLocalRole::Burn) {
                        self.burn_dct_token(&tx.token_identifier, &tx.amount);
                    }
                }
    ...
    /// Converts failed Ethereum -> Dharitri transactions to Dharitri -> Ethereum transaction.
    /// This is done every now and then to refund the tokens.
    ///
    /// As with normal Dharitri -> Ethereum transactions, a part of the tokens will be
    ...
    fn add_refund_batch(&self, refund_transactions: ManagedVec<Transaction<Self::Api>>) {
        ...
        for refund_tx in &refund_transactions {
            ...
            // "from" and "to" are inverted, since this was initially an Ethereum -> Dharitri tx
    ...
    /// Create an Dharitri -> Ethereum transaction. Only fungible tokens are accepted.
    ///
    ...
    /// Claim funds for failed Dharitri -> Ethereum transactions.
    ...
    fn claim_refund(&self, token_id: TokenIdentifier) -> DctTokenPayment<Self::Api> {
        let caller = self.blockchain().get_caller();
        ...
        self.send()
            .direct_dct(&caller, &token_id, 0, &refund_amount);

        DctTokenPayment::new(token_id, 0, refund_amount)
    }
    ...
    fn burn_dct_token(&self, token_id: &TokenIdentifier, amount: &BigUint) {
